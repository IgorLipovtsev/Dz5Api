...
@pytest.mark.parametrize("city_param", ["Anchorage", "Tucson", "Chandler", "Williams",])
def test_exact_city_in_all_brewery(city_param):
    ...
    all_brewery_by_city = requests.get(
        "https://api.openbrewerydb.org/breweries", params=city_params
    )
...
@pytest.mark.parametrize(
    "state_param", ["Pennsylvania", "New York", "Arizona", "Virginia",]
)
...
def test_search_dog_in_all_brewery():
    ...
    all_brewery = requests.get(
        "https://api.openbrewerydb.org/breweries/search", params=search_params
    )
